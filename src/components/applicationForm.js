...
const CoursesAdmissionForm = () => {
    ...
    //------ prospective data -----
    const [submitNewProspective, {
        data: prospectiveRegSuccessResponse,
        isLoading: prospectiveRegProcessing,
        isSuccess: prospectiveRegSucceeded,
        isError: prospectiveRegFailed,
        error: prospectiveRegError,
    }] = useItemRegistrerMutation()
    useEffect(() => {
        if (prospectiveRegSucceeded) {
            setStatus("✅ Prospective applicant registered successfully!");
            setLoading(false);
        ...
        } else if (prospectiveRegFailed) {
            setStatus("❌ Failed to register prospective. Please try again.");
            setLoading(false);
        }
    }, [prospectiveRegSucceeded, prospectiveRegFailed]);
    ...
    const handleSubmitToCloudRun = async (e) => {
        ...
        submitNewProspective({ entity: "prospective", data: payload })
    }
            ...
            <div className="space-y-4">
                ...
                <div>
                    <button
                        ...
                        type="submit"
                        disabled={prospectiveRegProcessing || loading}
                        className={`w-full mt-8 py-2 px-4 rounded-md text-white font-medium ${(prospectiveRegProcessing || loading)
                            ? 'bg-blue-300 cursor-not-allowed'
                            : 'bg-blue-600 hover:bg-blue-700'
                            }`}
                    >
                        {(prospectiveRegProcessing || loading) ? 'Submitting...' : 'Submit'}
                    </button>
                </div>
            </div>
