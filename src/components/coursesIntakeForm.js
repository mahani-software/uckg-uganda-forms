...
const CoursesIntakeForm = () => {
    ...
    const [allCourses, setAllCourses] = useState([{ guid: "ea324241315ac", courseName: "Database admin" }]);    //TODO: fetched from backend
    ...
    return (
        <div className="max-w-lg p-6 bg-white rounded-lg shadow-lg">
            ...
            <form onSubmit={handleSubmitToCloudRun} className="space-y-4">
                <div>
                    <label className="block mb-2 text-sm font-medium text-gray-700">Select Intake</label>
                    <select
                        name="intakeGuid"
                        value={intakeGuid}
                ...
                <div>
                    <FloatingLabelInput label="What challenge are you facing?" name="description" />
                </div>

                <div>
                    <label className="block mb-2 text-sm font-medium text-gray-700">Select Courses</label>
                ...
                <div>
                    <button
                        type="submit"
                        disabled={applicantRegProcessing || fileUploadProcessing || loading}
                        className={`w-full mt-8 py-2 px-4 rounded-md text-white font-medium ${(applicantRegProcessing || fileUploadProcessing || loading)
                            ? 'bg-blue-300 cursor-not-allowed'
                            : 'bg-blue-600 hover:bg-blue-700'
                            }`}
                    >
                        {(applicantRegProcessing || fileUploadProcessing || loading) ? 'Submitting...' : 'Submit'}
                    </button>
                </div>
            </form>
