...
const DocumentsInput = ({
    ...
    const validateFile = (file) => {
        ...
        const isValidType = acceptedDocTypes.some(type => {
            if (type.includes('*')) {
        ...
        if (!isValidType) {
            return `File type not supported. Accepted types: ${acceptedDocTypes.join(', ')}`;
        }
        return null;
    };
    ...
    const acceptedDocTypesCount = acceptedDocTypes.length;
    ...
    }, [files.length, maxFiles, maxFileSize, acceptedDocTypesCount, toast]);
    ...
    const uploadFile = async (uploadFile) => {
        ...
        if (uploadDocumentFn) {
            uploadDocumentFn({ formData, file: uploadFile.file, fileId: uploadFile.id })
        }
    };
    ...
    const uploadFileImmediately = async (newFile) => {
        ...
        if (uploadDocumentFn) {
            uploadDocumentFn({ formData, file: newFile })
        }
    };
